...
const DrugInfo = ({ drug }) => {
  ...
  const mapMixes = (cat) =>{
    return cat.map((el,i)=>{
      return (<li key={i} className="Bold">{el.name}</li>)
    })
  }
  console.log(drugCats)
  return (
    <div className={styles.DrugInfo}>
            ...
            {drug.duration.length > 0 &&
              drug.duration.map(method => (
                <li key={method.method} className={styles.table}>
                  <h4 className="Bold">{method.method}</h4>
                  <ul>
                    {method['Onset'] && <li><span>Onset:</span> <span>{method['Onset']}</span></li>}
                    {method['Duration'] && <li><span>Duration:</span> <span>{method['Duration']}</span></li>}
                    {method["After_effects"] && <li><span>After-Effects:</span> <span>{method['After_effects']}</span></li>}
                  </ul>
                </li>
              ))}
          </ul>
      ...
      {drug.interactions && drug.interactions.length > 0 && (<>
        ...
        {Object.keys(drugCats).map((cat,i)=>{
          ...
          return drugCats[cat] && (<Expandable
        header={capitalize(header)} classes={styles[cat]} key={i}>
